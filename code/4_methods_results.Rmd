...
newdata <- test.df %>% 
  ...
  bind_rows(readOGR(dsn="Data",layer="BristolBay") %>%  # Read in and mergee the crab shapefiles
  fortify() %>% 
  mutate(long2=ifelse(long<0,long+360, long),
    ...
    bind_rows(readOGR(dsn="Data",layer="St_Matthew_District") %>%
                fortify() %>% 
                mutate(long2=ifelse(long<0,long+360,long),
...
ggplot() + 
  geom_polygon(data=tidy(readOGR(dsn="Data",layer="AKbasemap")) %>% # Load basemap
                 mutate(long2=ifelse(long<0,long+360,long))
