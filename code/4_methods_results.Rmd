...
area<- readOGR(dsn="Data/Alaska_Marine_Management_Areas.gdb",layer="Alaska_Marine_Areas_dd",verbose=FALSE)
...
newdata <- test.df %>% 
  ...
  mutate(long2=ifelse(long<0,long+360, long),
    ...
    bind_rows(readOGR(dsn="Data",layer="St_Matthew_District",verbose=FALSE) %>%
                fortify() %>% 
                mutate(long2=ifelse(long<0,long+360,long),
...
ggplot() + 
  geom_polygon(data=tidy(readOGR(dsn="Data",layer="AKbasemap",verbose=FALSE)) %>% # Load basemap
                 mutate(long2=ifelse(long<0,long+360,long))
...
httr::content(httr::GET("https://apex.psmfc.org/akfin/data_marts/akmp/nmfs_area_crw_avg_sst?nmfs_area=640"), type = "application/json")%>% 
  bind_rows #By default the URL returns a list. "bind_rows" convert to data frame/tibble
```
...
updateddata <- httr::content(httr::GET('https://apex.psmfc.org/akfin/data_marts/akmp/nmfs_area_crw_avg_sst?nmfs_area=640&start_date=19850101&end_date=20211231'), type = "application/json") %>% 
  ...
  mutate(date=as_date(READ_DATE)) %>% 
  data.frame %>% 
  dplyr::select(date,meansst=MEANSST,NMFSAREA) #simplify the data frame for clarity.
```
...
httr::content(httr::GET('https://apex.psmfc.org/akfin/data_marts/akmp/ecosystem_sub_crw_avg_sst?ecosystem_sub=Northern%20Bering%20Sea&start_date=19850101&end_date=20211231'), 
